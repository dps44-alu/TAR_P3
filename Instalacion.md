# Guía de Instalación de Docker, ROS y ROS2

En este documento se presenta la guía de instalación de Docker, ROS y ROS2 para la realización de las prácticas de la asignatura Tecnologías y Arquitecturas Robóticas del Grado en Ingeniería Informática. 
...
Para poder ejecutar cualquier código fuente que vamos a usar en las prácticas es necesario tener un IDE, en este caso se recomienda la instalación de Visual Studio Code. 
...
En este caso, también nos va servir para poder conectarnos a los contenedores de Dcoker que generaremos con las imágenes de ROS y ROS2, de esta forma, se podrá trabajar de forma directa sobre ellos, pudiendo exprimir todas las ventajas y herramientas de forma directa.
...
Para ello, escriba los siguientes comandos APT para actualizar la base de paquetes e instalar los requisitos previos:
...
```
Observe que ninguno de los comandos genera salida.
...
A continuación, puede eliminar de forma segura la clave GPG descargada, que ya ha sido importada a su sistema.
...
Antes de proceder con la instalación de VS Code en nuestro sistema Ubuntu 24.04, debemos actualizar la lista de paquetes disponibles. Por lo tanto, ejecute el siguiente comando para actualizar el índice de repositorios de software de Ubuntu.
...
Todo está ya preparado para la instalación propiamente dicha. Para instalar VS Code en su sistema Ubuntu 24.04 LTS (Noble Numbat), ejecute el siguiente comando y espere a que se complete la instalación.
...
Para poder acceder a los contenedores que generaremos con Docker, es necesario instalar algunas Extensiones de Visual Studio. Para ello, iremos a la pestaña de extensiones de VS Code e instalaremos las extensiones de Docker y Remote Explorer.
...
En este caso se ha optado por instalar mediante el repositorio àpt`si desea usar otro método de instalación puede redirigirse mediante los enlaces anteriores.
...
Para instalar la una versión específica, ejecute:
```bash
...
3. Compruebe que la instalación se ha realizado correctamente ejecutando la imagen `hello-world`:
...
3. Cierre la sesión y vuelva a iniciarla para que se vuelva a evaluar su pertenencia al grupo.
...
4. Verifique que puede ejecutar comandos `docker` sin `sudo`.
```bash
...
Si inicialmente ejecutó comandos CLI de Docker utilizando `sudo` antes de agregar su usuario al grupo `docker`, puede ver el siguiente error:
```
...
Para solucionar este problema, elimine el directorio `~/.docker/` (se vuelve a crear automáticamente, pero se pierde cualquier configuración personalizada) o cambie su propiedad y permisos mediante los siguientes comandos.
...
Una vez seleccionado, simplemente habrá que aplicar los cambios y reiniciar el sistema. Para comprobar que la instalación ha funcionado correctamente, en una terminal ejecutar:
```bash
nvidia-smi
```
...
1. Configure el repositorio de producción:
```bash
...
Opcionalmente, configure el repositorio para utilizar paquetes experimentales:
```bash
sed -i -e '/experimental/ s/^#//g' /etc/apt/sources.list.d/nvidia-container-toolkit.list
```

2. Actualizar la lista de paquetes del repositorio:
```bash
sudo apt-get update
```
...
1. Configure el tiempo de ejecución del contenedor mediante el comando `nvidia-ctk`:
```bash
